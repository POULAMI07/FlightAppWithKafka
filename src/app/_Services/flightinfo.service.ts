...
export class FlightinfoService {
  ...
  getAirlines() {
    ...
    return this.httpClient.get(this.url3 + "/airlineController")
  } 
  
  getSchedules() {
    console.log("getting all schedule")
    return this.httpClient.get(this.url3 + "/scheduleController")
  }
  ...
  ShowSchedulebyId(id: any) {
    return this.httpClient.get(this.url3 + "/scheduleController/" + id);
  }
  ...
  SearchFlightByParams(s: string, d: string, dat: string) {
   ...
   //remember to change this to url2
  registrationBooking(bookObj: any) {
    return this.httpClient.post(this.url4 + "/bookingController", bookObj);
  }
